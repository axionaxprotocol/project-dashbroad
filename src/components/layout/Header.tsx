...
export const Header: React.FC<HeaderProps> = ({ projects, filters, onFilterChange, onRefresh, isRefreshing = false }) => {
  const { year, status, engineer, team, activeOnly } = filters;
    ...
    <header className="bg-[#0a192f] text-white shadow-md sticky top-0 z-30">
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-wrap items-center gap-2 pb-3 border-t border-slate-700/50 pt-3">
          ...
          {(year || status || engineer || team || activeOnly) && (
            <button
              onClick={() => onFilterChange({ year: '', status: '', engineer: '', team: '', activeOnly: false })}
